...
contract LayerA {
    // attributes of a relayer in a layer
    struct RelayerA {
        address relayer;
        string relayerId;
    ...
    // attributes of a bid in a layer
    struct Bid {
        address sender;
        address receiver;
        uint256 amount;
    ...
    mapping(string => RelayerA) public RelayersA;  // Maps relayerId to Relayer structure
    ...
    function placeBid(
        ...
        require(RelayersA[RelayerAdToId[relayer]].relayer == relayer, "Only the relayer can place bid");
        require(
            _amount > BidsMap[bidIdStr].amount,
            "Amount should be greater than the transaction amount."
        );
        require(BidsMap[bidIdStr].locked == true, "Bid is locked");
        require(BidsMap[bidIdStr].bought == false, "Bid is already bought");

        BidsMap[bidIdStr].bought = true;
        RelayersA[RelayerAdToId[relayer]].assignedBidId = bidIdStr;
    ...
    function isRelayerExists(address _relayer) external view returns (bool) {
